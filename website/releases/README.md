...
Each folder maps 1:1 to a button on the downloads page — drop a build in and
that button goes live automatically (empty folders show "coming soon"):

```text
releases/
├── windows/            newest file here = what /api/download?type=windows serves
│   ├── PolyForge-5.5.2-windows-amd64.exe   (older, kept for rollback)
│   ├── PolyForge-6.0.2-windows-amd64.exe   (newest — this one is served)
│   └── SHA256SUMS.txt                       (doc files are ignored)
├── windows-portable/   windows-upx/   windows-zip/   windows-arm64/
├── macos/  macos-dmg/  macos-zip/
├── linux/  linux-deb/  linux-rpm/  linux-targz/  linux-arm64/
└── flatpak/  snap/  aur/
```

Each folder also keeps a `SHA256SUMS.txt` (regenerated by the admin panel on
every upload/delete). The downloads page reads it to show the live SHA-256 of
the latest build behind the eye-toggle; empty folders render their button as a
dimmed "Coming soon".

`wails-build.ps1` already names the Windows binary `PolyForge-<version>-windows-amd64.exe`
(from the repo-root `VERSION` file), which is exactly the name this gateway
expects — just upload it into `releases/windows/`.
